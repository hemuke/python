        ...
        while True:
            with self.lock:
                ...
                if self.basket.full():
                    ...
                    print(
                        '>> 生产馒头：{0}，总共{1}个'.format(
                            name, self.basket.qsize()))
        ...
        while True:
            with self.lock:
                ...
                if self.basket.empty():
                    ...
                    time.sleep(random.randint(1, 5))
                    print(
                        '<< 消费馒头：{0}，总共{1}个'.format(
                            bread, self.basket.qsize()))
...
if __name__ == '__main__':
    ...
    for i in range(10):
        rest = pool_producer.apply_async(
            run_producer, args=(lock, counter, basket))
        print(rest)
    pool_producer.close()
